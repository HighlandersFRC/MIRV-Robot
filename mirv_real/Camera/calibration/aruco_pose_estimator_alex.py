...
class aruco_pose:
    ...
    def __init__(self):
        ...
        self.cal_file = rospy.get_param(
            '~calibration_file', 'calibrationSave_2.p')
        ...
        self.image_sub = rospy.Subscriber(
            self.ros_prefix + "/camera/rgb/image_raw", Image, self.callback)
        self.pose_pub = rospy.Publisher(
            self.ros_prefix + "/dse/pose_markers", PoseMarkers, queue_size=1)
    ...
    def callback(self, data):
        ...
        if (ids is not None):
            ...
            for i in range(len(rvecs)):
                ...
                get_frame_transform(poseZero, pose)

                pose = self.tfBuffer.transform(
                    pose, self.ros_prefix[1:] + '/base_link')
